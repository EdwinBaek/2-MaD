...
train_Path = './data/image2/train%s/' % IMAGE_SIZE
valid_Path = './data/image2/valid%s/' % IMAGE_SIZE
test_Path = './data/image2/test%s/' % IMAGE_SIZE
num_classes = 2
BATCH = 16
EPOCH = 100
...
class MyDataset(Dataset):
    def __init__(self, image_path, mode, transform=None):
        ...
        if mode == 'train':
            ...
            self.images = [image_path + '%s.jpg' % i for i in train_ID]
        elif mode == 'valid':
            self.labels = np.array(valid_Label, dtype=float)
            #self.labels = self.labels[:, np.newaxis]
            self.images = [image_path + '%s.jpg' % i for i in valid_ID]
        else:
            self.labels = np.array(test_Label, dtype=float)
            #self.labels = self.labels[:, np.newaxis]
            self.images = [image_path + '%s.jpg' % i for i in test_ID]
...
if __name__ == '__main__':
    ...
    summary(vgg19, (3, IMAGE_SIZE, IMAGE_SIZE))

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(vgg19.parameters(), lr=1e-4, weight_decay=1e-5, betas=(0.9, 0.99))
    ...
    for epoch in range(1, EPOCH + 1):
        ...
        for batch_idx, (data, label) in enumerate(train_loader):
            data, label = data.to(device), label.to(device, dtype=torch.int64)    # gpu setting

            optimizer.zero_grad()               # grad init
            output = vgg19(data)                # forward propagation
